...
print(f"  groq_model      : {s.groq_model!r}")
print(f"  groq_base_url   : {s.groq_base_url!r}")
print(f"  groq_api_key    : {'SET (gsk-...)' if s.groq_api_key.startswith('gsk-') else ('SET' if s.groq_api_key else 'NOT SET')}")
...
async def main() -> None:
    ...
    try:
        ...
        vector = await emb.embed_text(test_text)
    ...
    for text in test_memories:
        try:
            result = await mem_svc.save_memory(memory_text=text)
    ...
    for query in queries:
        print(f"\n  Query: {query!r}")
        try:
            results = await mem_svc.search_memory(query=query, top_k=5)
            if not results:
